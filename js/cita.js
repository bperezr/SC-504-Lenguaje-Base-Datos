function validarFormulario(event) {
  ...
  if (
    nombre.length === 0 ||
    nombre.length > 50 ||
    !/^[\w\sáéíóúñÁÉÍÓÚÑ]+$/.test(nombre)
  ) {
    mostrarMensajeError(
      "nombre",
      "Ingrese un nombre válido (máximo 50 caracteres, solo letras, espacios)"
    );
    return;
  }
  ...
  if (
    diaSemana >= 1 &&
    diaSemana <= 5 &&
    (horaSeleccionada < 8 || horaSeleccionada > horarioLimite)
  ) {
    mostrarMensajeError(
      "horario",
      "Seleccione un horario válido (lunes a viernes de 8 AM a 5 PM)"
    );
    return;
  }
  ...
  if (diaSemana === 6 && horaSeleccionada > 12) {
    mostrarMensajeError(
      "horario",
      "Seleccione un horario válido (de lunes a viernes de 8 AM a 5 PM, y sábados de 8 AM a 12 PM)"
    );
    return;
  }
...
var formularioParaCitas = document.getElementById("formularioParaCitas");
formularioParaCitas.addEventListener("submit", function (event) {
  event.preventDefault();
});
